...
def train(args):
    set_seed(args.seed)
    examples = parse_xml_sentences(args.train_xml, lang=args.lang)
    if len(examples) == 0:
...
# parser.add_argument("--output_dir", type=str, default="outputs/absa")
...
def main(args):
    ...
    if not args.dataset:
        raise ValueError("--train_xml required for training")
    train(args)
...
English train (your XML schema):

python3 src/absa_multilingual.py \
  --lang en \
  --train_xml "Restaurant_English_Training/training/Restaurants_Train.xml" \
  --output_dir outputs/en_absa \
  --epochs 2 --batch_size 4

Turkish train (your XML schema):

python3 src/absa_multilingual.py \
  --lang tr \
  --train_xml Restaurant_Turkish_Training/reviews.xml \
  --output_dir outputs/tr_absa \
  --epochs 5 --batch_size 4

=== Prediction ===
Inference (uses saved best threshold + labels):

- For English

python3 src/absa_multilingual.py --predict \
  --lang en \
  --model_dir outputs/en_absa \
  --text "Once we sailed, the top-notch food and live entertainment sold us on a unforgettable evening."

- For Turkish

python3 src/absa_multilingual.py --predict \
  --lang tr \
  --model_dir outputs/tr_absa \
  --text "Manzara şahane ama servis rezalet."
